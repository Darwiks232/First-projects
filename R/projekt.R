...
ggplot(data, aes(x = Płeć)) +  # Create a bar chart based on the 'Płeć' variable
  ...
  # Calculate the covariance between 'waznosc ceny' and 'waznosc jakosci'
cov_waznosc_ceny_jakosci <- cov(data$`waznosc ceny`, data$`waznosc jakosci`, use = "complete.obs")
cov_waznosc_ceny_jakosci
